...
public class ClientWorld extends World
{
    ...
    private void addEntityImpl(int entityIdIn, Entity entityToSpawn)
    {
        if (!Reflector.EntityJoinWorldEvent_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.EntityJoinWorldEvent_Constructor, entityToSpawn, this))
        {
            ...
            //TODO: Client
            if(entityToSpawn instanceof PlayerEntity && entityToSpawn != mc.player && !entityToSpawn.getName().getString().equalsIgnoreCase("Freecam")) {
            	ClientUtils.addChatMessage("Loaded player " + entityToSpawn.getName().getString() + " at: \2477" + MathHelper.floor(entityToSpawn.getPosX()) + ", " + MathHelper.floor(entityToSpawn.getPosY()) + ", " + MathHelper.floor(entityToSpawn.getPosZ()));
            }
    ...
    public void removeEntityFromWorld(int eid)
    {
        ...
        //TODO: Client
        if(entity instanceof PlayerEntity && entity != mc.player && !entity.getName().getString().equalsIgnoreCase("Freecam")) {
        	ClientUtils.addChatMessage("Unloaded player " + entity.getName().getString() + " at: \2477" + MathHelper.floor(entity.getPosX()) + ", " + MathHelper.floor(entity.getPosY()) + ", " + MathHelper.floor(entity.getPosZ()));
        }
    ...
    public static class ClientWorldInfo implements ISpawnWorldInfo
    {
        ...
        {
        	//TODO: Client
        	if(Minecraft.getInstance().hackedClient.getModuleManager().getModule("Night").getState()) {
        		this.dayTime = 20000;
        		return;
        	}
            this.dayTime = time;
