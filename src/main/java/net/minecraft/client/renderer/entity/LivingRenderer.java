...
public abstract class LivingRenderer<T extends LivingEntity, M extends EntityModel<T>> extends EntityRenderer<T> implements IEntityRenderer<T, M>
{
    ...
    {
    	//TODO: Client
    	if(Minecraft.getInstance().hackedClient.getModuleManager().getModule("Chams").getState() && entityIn instanceof PlayerEntity && entityIn != Minecraft.getInstance().player) {
    		GL11.glEnable(32823);
    ...
    protected boolean canRenderName(T entity)
    {
        ...
        //TODO: Client
        if(Minecraft.getInstance().hackedClient.getModuleManager().getModule("NameTags").getState()) return Minecraft.isGuiEnabled() && entity != Minecraft.getInstance().getRenderViewEntity() && !entity.isBeingRidden();
