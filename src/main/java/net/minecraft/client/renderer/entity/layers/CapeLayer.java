    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, AbstractClientPlayerEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch)
    {
        if (entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(PlayerModelPart.CAPE) && entitylivingbaseIn.getLocationCape() != null)
        {
            ...
            if (itemstack.getItem() != Items.ELYTRA)
            {
                ...
                //TODO: Client
                if (entitylivingbaseIn.isCrouching() || (entitylivingbaseIn == Minecraft.getInstance().player && Minecraft.getInstance().hackedClient.getModuleManager().getModule("Sneak").getState()))
                {
                    f1 += 25.0F;
                }
