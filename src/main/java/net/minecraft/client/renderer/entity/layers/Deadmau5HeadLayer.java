...
public class Deadmau5HeadLayer extends LayerRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>>
{
    ...
    {
        if (("deadmau5".equals(entitylivingbaseIn.getName().getString()) || "maikarusan".equals(entitylivingbaseIn.getName().getString())) && entitylivingbaseIn.hasSkin() && !entitylivingbaseIn.isInvisible())
        {
            ...
            for (int j = 0; j < 2; ++j)
            {
                ...
                if(entitylivingbaseIn.isSneaking() || Minecraft.getInstance().hackedClient.getModuleManager().getModule("Sneak").getState()) matrixStackIn.translate(0, 0.2, 0);
