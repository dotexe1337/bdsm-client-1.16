...
import cf.dotexe.bdsm.friend.Friend;
...
public class PlayerTabOverlayGui extends AbstractGui
{
    ...
    public void func_238523_a_(MatrixStack p_238523_1_, int p_238523_2_, Scoreboard p_238523_3_, @Nullable ScoreObjective p_238523_4_)
    {
        ...
        {
        	//TODO: Kinky O.o
        	String s = this.getDisplayName(networkplayerinfo).getString();
        	for(Friend f: this.mc.hackedClient.getFriendManager().getFriendsList()) {
        		if(s.toLowerCase().contains(f.getName().toLowerCase())) {
        			if(f.getName().toLowerCase().equalsIgnoreCase(mc.player.getName().getString().toLowerCase())) {
        				s = StringUtils.replaceIgnoreCase(s, f.getName(), "\247b" + Minecraft.getInstance().hackedClient.getFriendManager().getAliasName(f.getName()) + "\247r");
        			} else {
        				s = StringUtils.replaceIgnoreCase(s, f.getName(), "\2479" + Minecraft.getInstance().hackedClient.getFriendManager().getAliasName(f.getName()) + "\247r");
        			}
        		}
        	}
            int k = this.mc.fontRenderer.getStringWidth(s);
            ...
            {
            	//TODO: Kinky O.o
                k = this.mc.fontRenderer.getStringWidth(" " + s);
        ...
        for (int i5 = 0; i5 < i4; ++i5)
        {
            ...
            if (i5 < list.size())
            {
                ...
                //TODO: Kinky O.o
                String s = this.getDisplayName(networkplayerinfo1).getString();
            	for(Friend f: this.mc.hackedClient.getFriendManager().getFriendsList()) {
            		if(s.toLowerCase().contains(f.getName().toLowerCase())) {
            			if(f.getName().toLowerCase().equalsIgnoreCase(mc.player.getName().getString().toLowerCase())) {
            				s = StringUtils.replaceIgnoreCase(s, f.getName(), "\247b" + Minecraft.getInstance().hackedClient.getFriendManager().getAliasName(f.getName()) + "\247r");
                ...
                this.mc.fontRenderer.func_243246_a(p_238523_1_, new StringTextComponent(s), (float)k2, (float)l2, networkplayerinfo1.getGameType() == GameType.SPECTATOR ? -1862270977 : -1);
                ...
                if (p_238523_4_ != null && networkplayerinfo1.getGameType() != GameType.SPECTATOR)
                {
                    ...
                    {
                    	//TODO: Kinky O.o
                    	String s2 = gameprofile.getName();
                    	for(Friend f: this.mc.hackedClient.getFriendManager().getFriendsList()) {
                    		if(s2.toLowerCase().contains(f.getName().toLowerCase())) {
                    			if(f.getName().toLowerCase().equalsIgnoreCase(mc.player.getName().getString().toLowerCase())) {
                    				s2 = StringUtils.replaceIgnoreCase(s2, f.getName(), "\247b" + Minecraft.getInstance().hackedClient.getFriendManager().getAliasName(f.getName()) + "\247r");
                    			} else {
                    				s2 = StringUtils.replaceIgnoreCase(s2, f.getName(), "\2479" + Minecraft.getInstance().hackedClient.getFriendManager().getAliasName(f.getName()) + "\247r");
                    			}
                    		}
                    	}
                        this.func_175247_a_(p_238523_4_, l2, s2, l5, i6, networkplayerinfo1, p_238523_1_);
                    }
                }
